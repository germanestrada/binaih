...
interface MasterItem { id:string; code:string; title:string; icon:string; category:string; response_type:string; ai_enabled:boolean }
...
export default function TipoAuditoriaDetailPage() {
  ...
  const load = useCallback(()=>{
    ...
    ]).then(([d,m])=>{setDetail(d);setMasterItems(m.data??[]);setLoading(false)})
  },[typeId])
  ...
  const addFromMaster = async (master:MasterItem) => {
    const existing = detail?.items.map(i=>i.title)??[]
    if(existing.includes(master.title)){alert('Este ítem ya existe en el tipo de auditoría');return}
    ...
    const res = await fetch(`/api/admin/audit-types/${typeId}/items`,{
      ...
      body:JSON.stringify({title:master.title,section:'General',order_index:maxOrder+1,response_type:master.response_type,max_score:10,weight:1.0,required:true,ai_enabled:master.ai_enabled,active:true}),
    })
  ...
  const filteredMaster = masterItems.filter(m=>!masterFilter||m.title.toLowerCase().includes(masterFilter.toLowerCase())||m.category.toLowerCase().includes(masterFilter.toLowerCase()))
    ...
    <div style={{display:'flex',flexDirection:'column',height:'calc(100vh - 92px)'}}>
      ...
      <div style={{padding:'20px 24px',flex:1,overflowY:'auto'}}>
        <div style={{display:'flex',gap:10,marginBottom:20}}>
          ...
          <button onClick={()=>setModal('add-master')} style={BTN()}>+ Desde tabla maestra</button>
        </div>
      ...
      {/* Modal: ítem personalizado o editar */}
      ...
      {/* Modal: agregar desde tabla maestra */}
      {modal==='add-master'&&(
        <Modal title="Agregar desde tabla maestra" onClose={()=>setModal(null)}>
          <input style={INP} placeholder="Buscar ítem…" value={masterFilter} onChange={e=>setMasterFilter(e.target.value)}/>
          <div style={{maxHeight:400,overflowY:'auto',marginBottom:12}}>
            {filteredMaster.map((m,i)=>(
              <div key={m.id} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:i<filteredMaster.length-1?'1px solid var(--border2)':'none'}}>
                <div style={{fontSize:20,flexShrink:0}}>{m.icon}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:12,fontWeight:500,color:'var(--ink)'}}>{m.title}</div>
                  <div style={{fontSize:10,color:'var(--subtle)'}}>{m.category} · {RESPONSE_LABELS[m.response_type]} {m.ai_enabled?'· ✨ IA':''}</div>
                </div>
                <button onClick={()=>addFromMaster(m)} style={{background:'var(--ink)',color:'white',border:'none',padding:'5px 12px',borderRadius:'var(--r-sm)',fontSize:11,cursor:'pointer',fontFamily:'inherit',flexShrink:0}}>Agregar</button>
              </div>
            ))}
            {filteredMaster.length===0&&<div style={{padding:'20px',textAlign:'center',color:'var(--subtle)',fontSize:12}}>Sin resultados</div>}
          </div>
