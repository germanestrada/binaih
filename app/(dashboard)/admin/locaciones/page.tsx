...
export default function LocacionesAdminPage() {
  ...
  const del = async (l:Loc) => {
    if(!confirm(`¿Eliminar "${l.name}"? Solo es posible si no tiene auditorías.`))return
            ...
            return(
              <div key={l.id} style={{display:'flex',alignItems:'center',gap:14,padding:'13px 16px',borderBottom:i<locs.length-1?'1px solid var(--border2)':'none'}}>
                ...
                <div style={{display:'flex',gap:6}}>
                  ...
                  <button onClick={()=>del(l)} style={{background:'none',border:'1px solid var(--err-bg)',padding:'5px 10px',borderRadius:'var(--r-sm)',cursor:'pointer',fontSize:11,color:'var(--err)',fontFamily:'inherit'}}>Eliminar</button>
                </div>
              </div>
            )
          })}
