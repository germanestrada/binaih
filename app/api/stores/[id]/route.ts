...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [locRes, auditsRes, findingsRes, historyRes] = await Promise.all([
    fetch(`${url}/rest/v1/locations?id=eq.${id}&tenant_id=eq.${tenant}&select=*,location_types(name,icon)&limit=1`, { headers }),
    fetch(`${url}/rest/v1/audits?location_id=eq.${id}&select=id,score,status,completed_at,scheduled_at,auditor_id,notes&order=completed_at.desc&limit=10`, { headers }),
