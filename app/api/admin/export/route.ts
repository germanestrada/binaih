...
export async function GET(req: Request) {
  ...
  if (type === 'locations') {
    ...
    // Filtrar por locación si se pasa location_id
    let query = `/audits?tenant_id=eq.${tenant}&select=id,location_id,score,status,completed_at,scheduled_at,duration_min,notes&order=completed_at.desc`
  ...
  const filename = `tveo-${type}${locationId?`-${locationId}`:''}-${new Date().toISOString().slice(0,10)}.csv`
